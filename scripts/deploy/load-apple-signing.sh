#!/usr/bin/env bash
# Load Apple code-signing and notarization env for Mac Power Switcher macOS builds.
# Priority: existing env → APPLE_SIGNING_ENV → scripts/deploy/apple-signing.env
set -euo pipefail
...
if [[ -z "${APPLE_SIGNING_IDENTITY:-}" ]]; then
  if [[ -n "${APPLE_SIGNING_ENV:-}" ]] && load_env_file "$APPLE_SIGNING_ENV"; then
    ...
    echo "⚠ No signing env found (scripts/deploy/apple-signing.env)" >&2
  fi
fi
...
if has_notary_auth; then
  export MAC_POWER_SWITCHER_NOTARIZE="${MAC_POWER_SWITCHER_NOTARIZE:-1}"
else
  export MAC_POWER_SWITCHER_NOTARIZE="${MAC_POWER_SWITCHER_NOTARIZE:-0}"
